...
void STATE_SEND_DATA()
{
        ...
        if (EstablishConnection(String(cfg.server_ip), String(cfg.server_port)) == 0)
...
void STATE_TELEMETRY()
{
    ...
    if (cfg.no_send ==1){
        ...
        if (EstablishConnection(String(cfg.server_ip), String(cfg.server_port)) == 0)
        { //"heimdall.dedyn.io","1900" "130.149.67.141","4001"
                state = TRANS_SLEEP;
                Serial.println("going to sleep");
...
void STATE_STATION_MEASURING()
{
        ...
        // CODE
        if (GPS.fix)
        { // If GPS available start a measurement.
                delay(50);
                GPS.parse(GPS.lastNMEA());
...
*/
                ...
                if (station_measurement_counter % 10 == 0)
                { // Normal 40
